...
def run_saude_cnes(city: str, uf: str, ibge_cod_municipio: str, out_dir: str = "outputs"):
    ...
    out_city = os.path.join(out_dir, city)
    os.makedirs(out_city, exist_ok=True)
    out_csv = os.path.join(out_city, f"cnes_estabelecimentos_{ts}.csv")
    all_df.to_csv(out_csv, index=False)
    out_geojson = None
    if gdf is not None and "geometry" in gdf.columns and gdf.geometry.notnull().any():
        out_geojson = os.path.join(out_city, f"cnes_estabelecimentos_{ts}.geojson")
    ...
    if url and not all_df.empty:
        ...
        # Tabela geo + ├Ąndice espacial
        if out_geojson:
            gdf.to_postgis(
                "cnes_estabelecimentos_geo",
                con=eng,
                if_exists="replace",
                index=False,
                chunksize=5000,
            ...
            with eng.begin() as con:
                con.exec_driver_sql(
                    "CREATE INDEX IF NOT EXISTS ix_cnes_estab_geom ON cnes_estabelecimentos_geo USING GIST (geom)"
                )
